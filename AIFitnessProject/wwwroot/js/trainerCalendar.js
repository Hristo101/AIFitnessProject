        ...
        if (
            ...
            event.events.forEach((event) => {
                events += `<div class="event">
                    ...
                    ${event.workoutId ? `<div class="event-workout">Workout ID: ${event.workoutId}</div>` : ''}
                </div>`;
            });
        }
    });
...
attachWorkoutBtn.addEventListener("click", () => {
    workoutsList.classList.toggle("active");
});

document.querySelectorAll(".workout-item").forEach(workout => {
    workout.addEventListener("click", (e) => {
        if (e.target.classList.contains("details-btn")) {
            const workoutId = workout.dataset.workoutId;
            alert(`Details for workout ID: ${workoutId}`);
            return;
        }

        if (workout.classList.contains("selected")) {
            workout.classList.remove("selected");
            selectedWorkoutId = null;
        } else {
            document.querySelectorAll(".workout-item").forEach(w => w.classList.remove("selected"));
            workout.classList.add("selected");
            selectedWorkoutId = workout.dataset.workoutId;
        }
    });
});
